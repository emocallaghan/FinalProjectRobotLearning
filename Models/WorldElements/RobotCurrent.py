...
class IRSensor(abstractClass.Sensor):
    ...
    def getData(self,position,direction):
        ...
        distanceToWall = self.UltraSonicSensor.getDataNoNorm(position, direction)
        
        if(direction == 0):
            if(y > self.lightSource.y and self.correctOther(self.lightSource.x, x)):
                if(distanceToWall > abs(y-self.lightSource.y)):
                    print "Ultra"
                    print distanceToWall
                    print abs(y-self.lightSource.y)            
                    hasData = True 
        elif(direction == 90): 
            if(x > self.lightSource.x and self.correctOther(self.lightSource.y, y)):
                if(distanceToWall > abs(x-self.lightSource.x)):
                    print "Ultra"
                    print distanceToWall
                    print abs(x-self.lightSource.x) 
                    hasData = True
        elif(direction == 180):
            if(y < self.lightSource.y and self.correctOther(self.lightSource.x, x)):
                if(distanceToWall > abs(y-self.lightSource.y)):
...
class UltraSonicSensor(abstractClass.Sensor):
    ...
    def getDataNoNorm(self, position, direction):
        """Determines how far from closest wall"""
        x = position[0] 
        y = position[1] 
        wallsInFront = [] 
        wallToUse = None 
